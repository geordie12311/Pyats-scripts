...
@app.route("/hosts/<hostname>/running")
def get_host_running(hostname):
    nr = InitNornir(config_file="config.yaml")
    filtered = nr.filter(F(hostname=hostname))
    results = filtered.run(task=send_command, command="show run")
    my_list = [v.scrapli_response.result for v in results.values()]
    return render_template("running.html", my_list=my_list)


@app.route("/hosts/<location>/interfaces")
...
@app.route("/hosts/<hostname>/interfaces")
def get_host_interfaces(hostname):
    nr = InitNornir(config_file="config.yaml")
    filtered = nr.filter(F(hostname=hostname))
    results = filtered.run(task=send_command, command="show ip interface brief")
    my_list = [v.scrapli_response.result for v in results.values()]
    return render_template("interfaces.html", my_list=my_list)

@app.route("/hosts/<location>/interfaces")
def get_host_interface_loc(location):
    nr = InitNornir(config_file="config.yaml")
    filtered = nr.filter(F(location=location))
    results = filtered.run(task=send_command, command="show ip interface brief")
    my_list = [v.scrapli_response.result for v in results.values()]
    return render_template("interfaces.html", my_list=my_list)
