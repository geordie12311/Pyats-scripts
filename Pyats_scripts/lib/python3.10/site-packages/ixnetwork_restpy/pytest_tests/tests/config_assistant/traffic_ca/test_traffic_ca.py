...
def test_can_add_raw_traffic_over_lags(config_assistant):
    ...
    ethernet1 = (
        config.Topology.add(Ports=lag_1).DeviceGroup.add().Ethernet.add()
    )
    ethernet2 = (
        config.Topology.add(Ports=lag_2).DeviceGroup.add().Ethernet.add()
    )
    traffic_item = config.Traffic.TrafficItem.add(
        Name="Lag Traffic Item Sample", TrafficType="raw"
    )
    endpoint_set = traffic_item.EndpointSet.add(
        Destinations=lag_2, Sources=lag_1
    )
...
def test_can_add_raw_traffic_over_vports(config_assistant):
    ...
    traffic_item = config.Traffic.TrafficItem.add(
        Name="Raw Traffic Item Sample",
        TrafficType="raw",
        TrafficItemType="l2L3",
    )
    endpoint_set = traffic_item.EndpointSet.add(
        Sources=vport_1.Protocols, Destinations=vport_2.Protocols
    )
    config_assistant.commit()
...
def test_can_add_ipv4_traffic_over_scalable_source_multicast_receivers(
    config_assistant,
):
    config = config_assistant.config
    config_assistant.commit()
    ...
    ipv4_1 = (
        config.Topology.add(Vports=vport[0])
        .DeviceGroup.add()
        .Ethernet.add()
        .Ipv4.add()
    )
    ...
    ipv4_2 = (
        config.Topology.add(Vports=vport[1:4])
        .DeviceGroup.add()
        .Ethernet.add()
        .Ipv4.add()
    )
    ...
    scalable_sources = [
        {"arg1": ipv4_2.xpath, "arg2": 1, "arg3": 3, "arg4": 1, "arg5": 2},
        {"arg1": ipv4_2.xpath, "arg2": 1, "arg3": 3, "arg4": 9, "arg5": 2},
    ]
    multicast_receivers = [
        {
    ...
    traffic_item = config.Traffic.TrafficItem.add(
        Name="Ipv4 Traffic Item Sample",
        TrafficType="ipv4",
        TrafficItemType="l2L3",
    )
    endpoint_set = traffic_item.EndpointSet.add(
        ScalableSources=scalable_sources,
        MulticastReceivers=multicast_receivers,
    )
    config_assistant.commit()
    ...
    assert (
        len(traffic_item.ConfigElement.find().Stack.find(StackTypeId="ipv4"))
        == 1
    )
...
def test_can_add_ipv4_traffic_over_protocols(config_assistant):
    ...
    config_assistant.commit()
    ipv4_1 = (
        config.Topology.add(Vports=config.Vport.add()[-1])
        .DeviceGroup.add()
        .Ethernet.add()
        .Ipv4.add(Name="Ipv4 West")[-1]
    )
    ipv4_2 = (
        config.Topology.add(Vports=config.Vport.add()[-1])
        .DeviceGroup.add()
        .Ethernet.add()
        .Ipv4.add(Name="Ipv4 East")[-1]
    )
    traffic_item = config.Traffic.TrafficItem.add(
        Name="Ipv4 Traffic Item Sample",
        TrafficType="ipv4",
    ...
    endpoint_set = traffic_item.EndpointSet.add(
        Sources=ipv4_1, Destinations=ipv4_2
    )
...
def test_can_add_raw_traffic_over_custom_stack(config_assistant):
    ...
    config_element = config.Traffic.TrafficItem.find()[0].ConfigElement.find()[
        0
    ]
...
if __name__ == "__main__":
    pytest.main(
        [
            ...
            # "-k",
            # "test_can_add_raw_traffic_over_lags",
            __file__,
        ]
    )
